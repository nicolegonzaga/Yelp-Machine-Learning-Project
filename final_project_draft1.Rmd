...
I chose to do my machine learning project on this Yelp data set because I'm a very active Yelp user and always look for restaurants to try through Yelp. The goal of this project is to build a statistical learning model that can predict the star ratings of a review based on the review text and the cool, useful, and funny ratings.

.The main purpose of this project is to try different machine learning models that were learned in class and fit the best performing model to the testing set. During the exploratory data analysis process, I used a little bit of text mining and sentiment analysis to gain some insights about the data.
...
rf_best
```
The random forest model took the longest to run out of all 4. The top model seems to be the model with mtry = 3, trees=150, and mean = 0.7042445. This means that the Random forest model is fairly good at predicting star ratings. Also, random forest model is much better at predicting than the Naive Bayes Model. 
...
results %>% 
  arrange(-roc_auc)
```
Our best performing model is the Multinomial Logistic Regression model. So we can use this model to fit to our testing set. The Naive Bayes model performed the worst, which is not surprising since this model is known to be "bad" estimator.
